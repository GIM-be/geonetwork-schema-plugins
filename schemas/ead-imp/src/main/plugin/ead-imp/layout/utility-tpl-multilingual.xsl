...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="get-ead-imp-language">
  ...
  <xsl:template name="get-ead-imp-other-languages-as-json">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
			<xsl:variable name="mainLanguage">
				<xsl:call-template name="get-ead-imp-language"/>
			</xsl:variable>
			<lang>
  ...
  <xsl:template name="get-ead-imp-other-languages">
	<xsl:choose>
	  <xsl:when test="count($metadata/descendant::node()/*[@xml:lang!=''])>1">
          <xsl:for-each select="distinct-values($metadata/descendant::node()/*/@xml:lang)">
		      <xsl:variable name="langId" select="xslutil:threeCharLangCode(string(.))"/>
