...
:convertGent
 ...
 curl -X POST -u admin:psw4piloot --header "Accept: application/json" -F "file=@output\%~1.xml" -F "metadataType=METADATA" -F "uuidProcessing=OVERWRITE" -F "rejectIfInvalid=false" -F "transformWith=_none_" -F "assignToCatalog=false" -F "group=311" "http://geonetwork30.gim.be/geonetwork/srv/api/0.1/records"
 ENDLOCAL
 GOTO :eof